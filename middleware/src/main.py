...
from .schemas import ChatCompletionRequest, ToolExecutionRequest
from .auth import get_api_key
from .mcp_client import McpHost
from .native_tools import WEB_SEARCH_TOOL, NATIVE_TOOLS_REGISTRY
...
async def execute_tool(
    ...
    """
    Executes a specific tool (Native or MCP) hosted by the Middleware.
    This closes the 'Agentic Loop' by allowing the client to execute the
    instructions generated by the model.
    """
    tool_name = request.name
    arguments = request.arguments

    # 1. Try Native Tools
    if tool_name in NATIVE_TOOLS_REGISTRY:
        try:
            result = await NATIVE_TOOLS_REGISTRY[tool_name](**arguments)
            return {"status": "success", "output": result}
        except Exception as e:
            raise HTTPException(status_code=500, detail=f"Native Tool execution failed: {str(e)}")

    # 2. Try MCP Tools
    if mcp_host:
        try:
            # Check if tool exists in MCP (optional optimization, calling directly is fine)
